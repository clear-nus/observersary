    ...
    with xvfbwrapper.Xvfb():
        for iv in range(1, 6):
            for cv in checkpoints:
                ...
                for ia in range(1, 6):
                    for ca in checkpoints:
                        exp_name = f"{base_name}/evaluate/v{iv:02d}-c{cv:04d}/v{ia:02d}-c{ca:04d}-a01"
                        victim_model = f"results/{base_name}/train/v{iv:02d}/models/checkpoint_{cv * 1000}_steps.zip"
                        adversary_model = f"results/{base_name}/train/v{ia:02d}-c{ca:04d}-a01/models/best_model.zip"
                        print(f"Running {exp_name}...")
                        EvilSlimeEvaluationExperiment({"exp_name": exp_name, "victim_model": victim_model, "adversary_model": adversary_model, **args}).run()
